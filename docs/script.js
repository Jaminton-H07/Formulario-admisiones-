...
if (DEMO_MODE) {
  ...
  window.fetch = function (url, options) {
    ...
    if (u.includes('/admisiones')) {
      return Promise.resolve({
        ok: true, status: 201,
        json: () => Promise.resolve({ solicitudId: 'DEMO-' + Date.now(), mensaje: 'Solicitud creada exitosamente.' }),
      });
    }
    ...
    if (u.includes('/slots/')) {
      ...
      const slots = horas.map((hora, i) => ({ hora, disponible: i % 3 !== 1 }));
      return Promise.resolve({
        ok: true, status: 200,
    ...
    if (u.includes('/confirmar')) {
      return Promise.resolve({
        ok: true, status: 200,
        json: () => Promise.resolve({ mensaje: 'Cita confirmada exitosamente.' }),
      });
    }
